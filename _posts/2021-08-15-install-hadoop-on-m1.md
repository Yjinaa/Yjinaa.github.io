...
위에서 m1의 설치 경로는 opt 이하에 설치된다고 언급했다. 다른 파일들은 문제가 없는데, core-site.xml 파일을 보면 usrs/ 이하로 시작하는 경로가 보인다. 이 경로를 지금 설치된 경로로 수정해주어야 한다. 나는 처음에 이걸 몰랐어서 삽질을 엄청 했다... 수정하지 않으면 아래서 엄청난 오류가 발생한다. ㅠㅠ
...
**다시 포맷을 진행하고** (포맷 절대 하면 안된다... 이유는 아래...)아래 명령어로 ssh 키를 설정해주었다.
